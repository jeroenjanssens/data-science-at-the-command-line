...
Assume that we have a very long running command, such as the one shown in Example
\@ref(exm:slow-sh)
.
...
The `bc` tool, which we used in the beginning of the chapter, is not parallel by itself. However, we can parallelize it using `parallel`. The Data Science toolbox contains a tool called `pbc` [@pbc]. Its code is shown in Example
\@ref(exm:script-pbc)
.
