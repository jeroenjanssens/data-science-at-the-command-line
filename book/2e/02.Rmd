...
```{console setup_history, include=FALSE}
 export CHAPTER="02"
 rm -f /history/bash_history_${CHAPTER}
 export HISTFILE=/history/bash_history_${CHAPTER}
```
...
alias bat='bat --tabs 8 --paging never --terminal-width 76 --theme "Solarized (dark)"'
```
...
The output of a command-line tool is by default passed on to the terminal, which displays it on our screen. We can *pipe* the ouput of `seq` to a second tool, called `grep`, which can be used to filter lines. Imagine that we only want to see numbers contain a "3". We can combine `seq` and `grep` as follows:
