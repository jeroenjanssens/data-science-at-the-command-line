...
<5> Navigate the subdirectory *ch02*.
...
<!-- A long command can be broken up with either a backslash or a pipe symbol (`|`), which I'll explain below. -->
...
<!-- So far, you've been executing individual tools. -->
<!-- In that sense it's not so different from running individual GUI applications by clicking on their icon.  -->
<!-- In the next section I'll explain more about the different types of tools we're going to use in this book. -->
...
Most often, you'll use a different source of input, such as other tools and files.
...
```{r diagram-essential-pipe-redirect, echo=FALSE, fig.cap="The output from a tool can be piped to another tool or redirected to a file.", fig.align="center"}
...
```{r diagram-essential-devnull, echo=FALSE, fig.cap="Redirecting *`stderr`* to */dev/null*", fig.align="center"}
