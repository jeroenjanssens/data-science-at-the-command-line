...
While this book discusses over 90 command-line tools, it’s not the tools themselves that matter most.
...
While the command line as a technology and as a way of working is timeless, some of the tools discussed in the first edition have either: (1) been superseded by newer tools (e.g., `csvkit` has largely been replaced by `xsv`), (2) been abandoned by their developers (e.g., `drake`), or (3) been suboptimal choices (e.g., `weka`).
...
* [Chapter 6](#chapter-6-managing-your-data-workflow) has been rewritten from scratch. Instead of `drake` I now use `make` to do project management. `drake` is no longer maintained and `make` is much more mature and very popular with developers.
