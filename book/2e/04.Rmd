...
The advantages of a command-line tool are that you do not have to remember the entire one-liner and that it improves readability if you include it into some other pipeline.
...
Because we use Bash as our shell, the script will be written in the programming language Bash.
...
However, command-line tools do not need to have an extension.
...
The reason we cannot execute our file directly is that we do not have the correct access permissions.
...
It is not a good idea to leave it out, as we have done in the previous step, because then the behavior of the script is undefined.
The Bash shell uses the executable */bin/sh* by default.
Other shells may have different defaults.
In our case we want to use `bash` to interpret our commands.
...
So, the solution is to simply remove the `curl` command from the script.
...
Of course, if you intend to use a command-line tool only for own projects, then there are no limits to how specific you can be.
```
...
However, with larger scripts and a few more command-line arguments such as *\$2* and *\$3*, the code becomes more readable when you use named variables.
...
In a fresh Data Science Toolbox, the *PATH* looks like this:
...
(Sure, not every feature of the Bash programming language was employed, but the interpreter still was `bash`.) As you know by now, the command line is language agnostic, so we do not necessarily have to use Bash for creating command-line tools.
...
I cannot offer a complete introduction to either language, so we assume that you have some familiarity with Python and or R.
Programming languages such as Java, Go, and Julia, follow a similar pattern when it comes to creating command-line tools.
...
Third, the command-line tool needs to be more robust.
...
Command-line tools in Python and R need to specify `python` and `Rscript` [@R], respectively, as the interpreter after the shebang.
...
(There are a few command-line tools which require the complete data before they write any data to standard output, like `sort`.) This means the pipeline is blocked by such command-line tools.
This does not have to be a problem when the input data is finite, like a file.
...
I advise you to keep an eye out for opportunities to create one.
The next chapter covers the second step of the OSEMN model for data science, namely scrubbing data.


## For Further Exploration

- Adding help to your tool becomes important when the tool has many options to remember. Even more so when you want to share your tool with others. `docopt` is a language-agnostic approach to provide help and define the possible options that your tool accepts. There are implementations available in just about any programming language including Bash, Python, and R.
- If you want to learn more about programming in Bash, there are two books that I can recommend: *Classic Shell Programming* by Arnold Robbins and Nelson Beebe and *Bash Cookbook* by Carl Albing and JP Vossen.
- Writing a robust and safe Bash script is quite tricky. [ShellCheck](https://www.shellcheck.net/) is an online tool that will check your Bash code for any possible mistakes and vulnerabilities. There's also a command-line tool available if you rather not post your code on a website.
