...
display heights.png#! enter = FALSE
```

```{console, include = FALSE}
C-c #! literal = FALSE
mv heights.png /images/docker-starwars-heights.png
```

```{r starwars-heights, echo=FALSE, fig.cap="Distribution of heights per species in Star Wars", fig.align="center"}
knitr::include_graphics("images/docker-starwars-heights.png")
```
...
3. The `.SHELLFLAGS` line makes Bash more strict, which is considered a best practice.[^1] For example, because of this, the pipeline in the rule for target `top10` now stops as soon as there is an error.
...
[^1]: See http://redsymbol.net/articles/unofficial-bash-strict-mode/ for more information.
