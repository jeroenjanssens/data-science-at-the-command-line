...
It's important that our steps can be reproduced, by us or by others. When you continue with a project from some time ago, chances are that you have forgotten which commands you ran, from which directory, on which files, with which parameters, and in which order.
...
The space in front of the rule is a single tab character. `make` is picky when it comes to white space.
...
I need to add the `-f` option because the configuration file isn't actually called *Makefile*.
...
Not only will it make your life easier, but you will also start thinking about your data workflow in terms of steps.
Just as with your command-line toolbox, which you expand over time, the same holds for `make` workflows.
...
It is a very interactive and iterative process. After a while, it is easy to forget which steps you have taken to get the desired result.
It is therefore very important to document your steps every once in a while.
