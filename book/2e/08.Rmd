...
<!-- TODO: SHOULD discuss resume mechanism -->
...
C-C#! literal=FALSE
parallel --jobs 1 --keep cowsay ::: 1 2 3
```
...
cowsay 3 > /dev/null
```
<1> Because the output is the same as before, I redirect it to */dev/null* to supress it.
...
`parallel` allows you to *tag* the output with the `--tag` option:
...
<!-- #TODO: Do I mention --bar somewhere? -->
...
The command `aws ec2 describe-instances` returns a lot of information about all your EC2 instances in JSON format (see <https://awscli.amazonaws.com/v2/documentation/api/latest/reference/ec2/describe-instances.html>).
...
seq 2 | parallel --slf hostnames echo 2>&1
 echo 'bash: parallel: command not found'
 echo -n 'parallel: Warning: Could not figure out number of cpus on' &&
 echo ' ec2-54-88-122-140.compute-1.amazonaws.com (). Using 1.'
 echo '1'
 echo '2'
```
...
```{console, remove="echo"}
seq 1000 | parallel -N100 --pipe --slf hosts  "(hostname; wc -l) | paste -sd:"#! enter=FALSE
C-C#! literal=FALSE
...
```{console, remove="echo"}
seq 1000 | parallel -N100 --pipe --slf hosts "paste -sd+ | bc" | paste -sd+ | bc#! enter=FALSE
C-C#! literal=FALSE
echo '500500'
```
...
seq 1000 | parallel -N100 --basefile add --pipe --slf hostnames './add' | ./add #! enter=FALSE
C-C#! literal=FALSE
echo '500500'
```

Of course, summing 1000 numbers is only a toy example.
It would've been much faster to do this locally.
...
You don’t have that data on your local machine yet, so let’s first obtain it from <https://data.cityofnewyork.us/> using their free API:
