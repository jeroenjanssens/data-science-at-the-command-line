...
<2> The variable *`i`* is assigned the value "0" in the first iteration, "1" in the second iteration, and so forth.
...
```{console bat_slow, callouts=list("RANDOM", "sleep", "ts")}
bat slow.sh
```
...
```{console}
find /data/ch03 -type f | parallel echo '{#}\) \"{}\" has basename \"{/}\"'# <1>
```
