...
These algorithms come from the field of machine learning, so I'm going to change the vocabulary a bit.
...
In this chapter, I’ll be using a dataset of wine tasters' notes on red and white varieties of Portuguese wine called vinho verde.
...
Just out of curiosity, let’s see what the how the distribution of quality looks like for both red and white wines.
...
In this section, I’ll be using Vowpal Wabbit, or `vw`, to create a model that predicts the quality of the white wine, based on their physicochemical properties.
...
Vowpal Wabbit, or `vw` as the tool is invoked, has its own data format.
The tool `csv2vw` can, as its name implies, convert CSV to this format.
The `--label` option is used to indicate which column contains the labels.
Let's examine the `vw` format:
...
The line starts with the label, followed by a pipe symbol and then feature name/value pairs.
...
The model, also known as the *final regressor*, is stored in the file *wine.model*.
...
!! Only testing, label is not being used!
...
Using `rush`, I visualize the relationship between the observed values and the predicted values:
...
<!-- TODO: CH: Similar to the previous section, a quick wrap-up would tie this section up nicely. -->
...
vw \
--data wine-train.vw \
--final_regressor wine2.model
vw \
--data wine-test.vw \
--initial_regressor wine2.model \
--testonly \
--predictions predictions \
--quiet
paste -d, predictions <(cut -d '|' -f 1 wine-test.vw) |
