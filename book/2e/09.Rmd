...
```{console cp_cache_wine, include=FALSE}
...
knitr::include_graphics("images/wine-alchohol-vs-quality.png")
```
...
Although Tapkee is mainly a library that can be included in other applications, it also offers a command-line tool `tapkee`[@tapkee].
...
```{console plot_pca}
rush plot --x pc1 --y pc2 --color type wine-both-pca.csv > wine-pca.png
...
rush plot --x observed --y predicted --geom smooth > wine-regression.png
...
<3> Add "line numbers" using `nl` [@nl] in front of each line
...
I run the experiment using `skull`:
