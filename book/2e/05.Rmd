...
Two chapters ago, in Step 1 of the OSEMN model for data science, we looked at how to obtain data from a variety of sources.
It’s not uncommon for this data to have, say, missing values, inconsistencies, errors, weird characters, or just uninteresting columns.
Sometimes you only need a specific portion of the data. And sometimes you need the data to be in a different format.
In those cases, you have to clean, or *scrub*, the data before you can move on to Step 3: Exploring Data.

The data you obtained can come in a variety of formats. Common ones are plain text, CSV, JSON, HTML, and XML.
Since most command-line tools operate on one format only, it's worthwhile to be able to convert data from one format to another.

Once your data is in the format you want it to be, you can apply common scrubbing operations. These include filtering rows, replacing values, and merging multiple datasets.
The command line is especially well-suited for these kind of operations, as there exist many powerful command-line tools that are optimized for handling large amounts of data.
Tools that I’ll discuss in this chapter include classic ones such as: `cut` [@cut] and `sed` [@sed], and newer ones such as `jq` [@jq] and `xmlstarlet` [@xmlstarlet].

I'll introduce the command-line tools on a need-to-use basis.
You'll notice that sometimes you can use the same command-line tool to perform multiple operations, and vice versa, multiple command-line tools to perform the same operation.
This chapter is more structured like a cookbook, where the focus is on the problems or recipes, rather than on the command-line tools.
...
You'll start the following files:
...
It's not only the *input* data that may need scrubbing.
The transformations that you'll learn in this chapter can be useful at any step of your pipeline.
Generally speaking, it depends on whether the previous tool generates output that can immediately be used by the next tool.
If so, then you can chain the two tools using the pipe operator (`|`).
...
Imagine that you have the first 100 items of a fizzbuzz sequence and that you'd like to visualize how often the words *fizz*, *buzz*, and *fizzbuzz* appear using a bar chart.
...
<2> Using `sort` and `uniq` this way is a common way to count lines and sort them in descending order

Notice that `sort` is used twice.
The first `sort` is needed because `uniq` assumes its input data to be sorted.
In a way, this is an intermediate transformation, albeit a subtle one.
The next step would be to visualize the counts using `Rio`.
However, since `Rio` expects the input data to be in CSV format, this requires a less subtle transformation first.
`awk` [@awk] is able to add a header, flip the two fields, and separate them by a comma:
...
Now you're ready to use `Rio` to create a bar chart.
The expression passed to `Rio` contains syntax that belongs to `ggplot2`, a powerful visualization package for R that I cover in Chapter 7.
...
```{r fb_image, echo=FALSE, fig.align="center", out.width="90%"}
knitr::include_graphics("images/fb.png")
```

Although this example is a bit contrived, it reveals a pattern that is common while working at the command line.
...
In this section I describe common scrubbing operations for plain text.
Plain text refers to a sequence of human-readable characters and optionally, some specific types of control characters (for example a tab or a newline)[@plaintext].
Examples include: e-books, emails, log files, and source code.

For the purpose of this book, I assume that the plain text contains data, but that it has no clear tabular structure (like CSV does) or nested structure (like JSON, XML, and HTML do).
I discuss those formats later in this chapter.
...
This means that from the input data, each line will be evaluated whether it may be passed on as output.
...
You can print the first 3 lines using either `head` [@head], `sed`, or `awk`:
...
Notice that with `tail` you have to add one. Think of it as the line from which you want to start printing.
...
You can print specific lines using a either `sed`, `awk`, or a combination of `head` and `tail`. Here I print lines 4, 5, and 6:
...
Sometimes you want to extract or remove lines based on their contents.
Using `grep`, the canonical command-line tool for filtering lines, you can print every line that matches a certain pattern or regular expression.
...
With the `-v` option you invert the matches, meaning, `grep` prints the lines which don't match the pattern.
The regular expression below matches lines that contain white space, only.
So with the inverse, you can count the number of non-empty lines:
...
The main purpose of `sample` [@sample] is to get a subset of the data by outputting only a certain percentage of the input on a line-by-line basis.
...
This percentage can also be specified as a fraction (*1/100*) or as a probability (*0.01*).

`sample` has two other purposes, which can be useful when you’re debugging.
First, it’s possible to add some delay to the output.
This comes in handy when the input is a constant stream (for example, the Wikipedia stream we say in Chapter 3), and the data comes in too fast to see what’s going on.
Secondly, you can put a timer on `sample`.
This way, you don’t have to kill the ongoing process manually.
For example, to add a 1 second delay between each output line to the previous command and to only run for 5 seconds, you would use:

```{console}
seq -f "Line %g" 1000 | sample -r 1% -d 1000 -s 5#!enter=FALSE
```
...
In fact, this argument holds any command-line tool that reduces data, like `head` and `tail`.
Once you’re confident your pipeline works, you simply take it out of the pipeline.
