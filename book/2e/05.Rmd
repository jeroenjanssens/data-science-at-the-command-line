...
This chapter is all about the second step of the OSEMN model: *scrubbing* data.
...
For example, you may have obtained some JSON data from an API while you need it to be in CSV format to create a visualization.
...
Once the data is in the desired format, there could still be issues like missing values, inconsistencies, weird characters, or simply unnecessary parts.
Those can be fixed by applying transformations such as applying filters, replacing values, and combining multiple files.
...
Tools that I’ll discuss in this chapter include classic ones such as: `grep` [@grep] and `awk` [@awk], and newer ones such as `jq` [@jq] and `xmlstarlet` [@xmlstarlet].

You'll notice that sometimes you can use the same command-line tool to perform several operations, and vice versa, multiple tools to perform the same operation.
...
I'll start with the following files:
...
The instructions to get these files are in Chapter 2.
...
Here’s the source code for completeness:
...
The command-line tools that I’ll be using here are: `curl`, `scrape` [@scrape], `xml2json` [@xml2json], `jq` [@jq], and `json2csv` [@json2csv].
