...
Don't worry about the actual syntax just yet; I'll explain that in a moment.
...
If you run `display` you'll likely find that it doesn't work for you.
In fact, displaying images from the command line is tricky in general.

Use option `--dpi` to change dpi.

This is the pattern that I follow. First ansi then png.

- Depending on how your reading this book .

### Viewing Images

When it comes to viewing images that you create with the command line, you have at least four options.
\@ref(fig:screenshot-display-ansi-and-inline) is awesome.

```{r screenshot-display-ansi-and-inline, echo=FALSE, fig.cap="Viewing an image in the terminal via ASCII characters and ANSI escape sequences (top) and via the iTerm2 inline images protocol (bottom)", fig.align="center"}
knitr::include_graphics("images/screenshot_display_ansi_and_inline.png")
```

\@ref(fig:screenshot-display-preview-and-browser) is awesome too.

```{r screenshot-display-preview-and-browser, echo=FALSE, fig.cap="Viewing an image externally via a file explorer and an image viewer (left) and via a webserver and a browser (right)", fig.align="center"}
