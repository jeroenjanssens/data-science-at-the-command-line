...
These two shortcuts can save a lot of typing but I have to admit that they're not easy to remember.
...
Here's a violin plot demonstrating this (see also \@ref(fig:plot-labels-image).
...
Facets allow you to slice the data on one or two columns and place multiple visualizations side-by-side.
It requires a syntax which is unfortunately out of scope to explain.
Here, I also demonstrate how to use a constant value instead of the values of a particular feature using the `I()` syntax. 

I'm skipping the textual representation because there's just too much going for it to make sense (see \@ref(fig:plot-facets-image)).
...
Adding facets to your visualization can be useful for incorporating one or to more features.
This visualization is showing six features at once, which is really pushing it.
In this case you might consider to apply a dimensionality reduction algorithm before you visualize it.
...
Feel free to read that chapter at a later point if you can't wait to start modeling your data in [Chapter 9](#chapter-9-modeling-data).
