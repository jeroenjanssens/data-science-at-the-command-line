...
The Docker container has a tool called `servewd`, which serves the current working directory.
You only need to run `servewd` once from a directory (for example, */data/*) and it will happily run in the background.
You can then visit *localhost:8000* in your browser and access the contents of that directory and all of its subdirectories. 
This option potentially also works when you're logged in to a remote machine.
