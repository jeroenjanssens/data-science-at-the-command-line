...
```{console plot_demo_png, callout=1}
rush plot --x bill --y tip --color size --facets '~day' tips.csv > tips.png
display tips.png
```
```{r plot_demo_image, echo=FALSE, fig.cap="Demo image", fig.align="center", out.width="90%"}
knitr::include_graphics("images/tips.png")
```

(Don't worry about this syntax of `rush` or the contents of the image; that'll come later.)
...
\@ref(fig:screenshot-display-ansi-and-inline) shows options 1 and 2, which display the image inside the terminal itself.
...
Option 1 uses ASCII characters and ANSI escape sequences to display a textual representation of the image. 

Option 2 uses the iTerm2 inline images protocol.
iTerm2 is a terminal only available on macOS. 


\@ref(fig:screenshot-display-preview-and-browser) shows options 3 and 4, which display the image outside the terminal.
