...
Before we dive into creating data visualizations I'd first like to explain how you can display them.
...
Once you've plotting something, you can visit *localhost:8000* in your browser and access the contents of that directory and all of its subdirectories. 
...
Option 4 potentially also works when you're working on a remote machine.
...
However, as you may have noticed, I had used a very different command to create *tips.png*:
...
This allows us to avoid learning R and the grammar of graphics while still creating beautiful basic plots.
...
For now, while we're at the command line and don't want to fiddle too much about any single visualization, let's take that shortcut.
...
The plotting options that take a *`<name>`* ...

plot loads the `ggplot2` package

stands on the shoulder of giants: ggplot2 and devoutansi package.


<!-- Use option `--dpi` to change dpi. -->
<!-- This is the pattern that I follow. First ansi then png. -->

Let's create a few basic plots
...
A bar chart is useful for displaying the value counts of a categorical feature. 
...
\@ref(fig:plot_bar_image) shows the graphical visualization, which is created by `rush plot` when the output is redirected to a file.

```{console plot_bar_png}
rush plot --x time tips.csv > plot-bar.png
display plot-bar.png
```
```{r plot_bar_image, echo=FALSE, fig.cap="A bar chart", fig.align="center", out.width="90%"}
...
\@ref(fig:plot_histogram_image) shows again the graphical visualization. 
Below, I use two syntax shortcuts.
The double exclamation mark (`!!`) gets replaced with the previous command.
The new command gets printed by the Z shell so you know what it executes.
...
This two shortcuts can save a lot of typing but I have to admit that they're not easy to remember.

```{console plot_histogram_png}
!! > plot-histogram.png
display !$
```
```{r plot_histogram_image, echo=FALSE, fig.cap="A histogram", fig.align="center", out.width="90%"}
...
In this case, the textual representation really shows its limitations when compared to the visual representation in \@ref(fig:plot_density_image).

```{console plot_density_png}
rush plot --x tip --fill time --geom density tips.csv > plot-density.png
display plot-density.png
```
```{r plot_density_image, echo=FALSE, fig.cap="A density plot", fig.align="center", out.width="90%"}
...
There are 52 geometry types available in `ggplot2`:

```{console geoms}
rush run --library ggplot2 'apropos("^geom_")' | trim
```
...
This happy little accident becomes clearer in the visual representation in \@ref(fig:plot_accident_image).
...
```
```{r plot_accident_image, echo=FALSE, fig.cap="A happy little accident", fig.align="center", out.width="90%"}
knitr::include_graphics("images/plot-accident.png")
```

There's [cheat sheet for `ggplot2`](https://ggplot2.tidyverse.org/) which provides a good overview of the available geometry types. 


### Creating Scatter Plots

Because the rows in *tips.csv* are independent, it's better to visualize the relationship between the *tip* and the *bill* with a scatter plot.
This happens to be the default geometry when specifying two continuous features:
...
See \@ref(fig:plot_scatter_image) for the visual representation.

```{console plot_scatter_png}
rush plot --x bill --y tip --color time tips.csv > plot-scatter.png
display plot-scatter.png
```
```{r plot_scatter_image, echo=FALSE, fig.cap="A scatter plot", fig.align="center", out.width="90%"}
...
`rush plot` cannot handle transparency, so a visual representation (see \@ref(fig:plot_trend_image) is much better in this case.

```{console plot_trend_png}
rush plot --x bill --y tip --color time --geom smooth tips.csv > plot-trend.png
display plot-trend.png
```
```{r plot_trend_image, echo=FALSE, fig.cap="Trend lines", fig.align="center", out.width="90%"}
knitr::include_graphics("images/plot-trend.png")
```

If you like to visualize the original points along with the trend lines, you need to resort to writing `ggplot2` code with `rush run` (see \@ref(fig:plot_trend_points_image)).
...
```
```{r plot_trend_points_image, echo=FALSE, fig.cap="Trend lines and original points combined", fig.align="center", out.width="90%"}
...
factor is needed because otherwise
...
```
```{r plot_boxplot_image, echo=FALSE, fig.cap="A Box plot", fig.align="center", out.width="90%"}
...
default label is based on column specification.
...
```
```{r plot_labels_image, echo=FALSE, fig.cap="A violin plot with a title and labels", fig.align="center", out.width="90%"}
...
already seen it in the two screenshots from the previous section
weird syntax (out of scope). Also I().


```{console plot_facets_ansi, trailing_spaces=TRUE, remove=c(-2, -1)}
rush plot --x size --y bill --color smoker --size size --facets 'time ~ day' --alpha 'I(0.7)' --title 'Bubble chart' tips.csv
```


```{console plot_facets_png}
rush plot --x size --y bill --color smoker --size size --facets 'time ~ day' --alpha 'I(0.7)' --title 'Bubble chart' tips.csv > plot-facets.png
display plot-facets.png
```
```{r plot_facets_image, echo=FALSE, fig.cap="A bubble chart with eight facets", fig.align="center", out.width="90%"}
knitr::include_graphics("images/plot-facets.png")
```

No lunch on Saturday and Sunday
Days out of order. Need more R for this.
...
you need ggplot2
multiple layers, (scatter plus trend lines)
theming
many extensions available for ggplot2
plotnine?
