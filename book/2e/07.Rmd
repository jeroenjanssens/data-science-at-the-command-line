...
\@ref(fig:plot-histogram-image) shows again the graphical visualization. 
...
rush plot --x bill --y tip --color smoker --size size --facets 'time ~ day' --alpha 'I(0.7)' --title 'Bubble chart' tips.csv
```

\@ref(fig:plot-facets-image)

```{console plot_facets_png}
rush plot --x bill --y tip --color smoker --size size --facets 'time ~ day' --alpha 'I(0.7)' --title 'Bubble chart' tips.csv > plot-facets.png
display plot-facets.png
```
```{r plot-facets-image, echo=FALSE, fig.cap="A bubble chart with eight facets", fig.align="center", out.width="90%"}
...
<!-- ``` -->
<!-- ```{r plot-hist-image, echo=FALSE, fig.cap="Histogram", fig.align="center", out.width="90%"} -->
