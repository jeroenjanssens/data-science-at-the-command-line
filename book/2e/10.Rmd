...
It's the 800-pound gorilla you turn to when its impossible to fit your data in memory.
Spark itself is written in Scala, but using PySpark and SparkR you can also interact with it using Python and R, respectively.
...
In the session below, I start a Spark shell and count the number of occurrences of *alice* in the book *Alice’s Adventures in Wonderland*.
...
```{block2, type="rmdwarning"}
The `pipe()` transformation is also available in PySpark and SparkR.
```
...
Bill Chambers and Matei Zaharia (the original author of Spark) mention in their book *Spark: The Definitive Guide* that "[t]he `pipe` method is probably one of Spark's more interesting methods."
I too find it interesting that the developers of Spark added the ability to leverage a 50-year old technology.
