...
In total, `curl` supports more than 20 protocols (see https://ec.haxx.se/protocols/protocols-curl). To download from an FTP server, which stands for File Transfer Protocol, you use `curl` the same way. Here I download the file *welcome.msg* from *ftp.gnu.org*:
...
Common file extensions of compressed archives are: *.tar.gz*, *.zip*, and *.rar*. To decompress these, you would use the command-line tools `tar` [@tar], `unzip` [@unzip], and `unrar` [@unrar], respectively. (There are a few more, though less common, file extensions for which you would need yet other tools.)
...
Luckily, there is a command-line tool called `in2csv` [@in2csv], that converts Microsoft Excel spreadsheets to CSV files. CSV stands for comma-separated values. Working with CSV can be tricky because it lacks a formal specification. [@shafranovich2005common] defines the CSV format according to the following three points:
...
`sql2csv` needs two arguments: `--db`, which specifies the database URL, of which the typical form is: `dialect+driver://username:password@host:port/database` and `--query`, which contains the `SELECT` query. For example, given an SQLite database that contains the standard datasets from R[^1] , I can select all the rows from the table `mtcars` and sort them by the `mpg` column as follows:
...
[^1]: See https://github.com/r-dbi/RSQLite/blob/master/inst/db/datasets.sqlite.
