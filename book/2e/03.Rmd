...
If you have one or more files on your local computer, and you want to apply some command-line tools to it, all you have to do is copy or move the file to that mapped directory. Let’s assume that you have a file called *logs.csv* in your Downloads directory.
...
If you are running Linux or macOS, open execute the following command on your operating system (and not inside the Data Science Toolbox):
...
This data comes in various forms and can be downloaded using various protocols.
The command-line tool `curl` [@curl] can be considered the command line's Swiss Army knife when it comes to downloading data from the Internet.

### Introducing `curl`

When you access a URL, which stands for *uniform resource locator*, through your browser, the data that's being downloaded can immediately be interpreted by the browser. For example, an HTML file may be nicely rendered, a video file is perhaps automatically played, and a PDF file may be immediately shown. However, when you use `curl` to access a URL, the data is downloaded and by default printed to standard output. Nothing special is being done by `curl`, but other command-line tools can be used to process this data further.

The easiest invocation of `curl` is to specify a URL as a command-line argument. Let's, for example, download a certain article from Wikipedia:

```{console curl_simple}
curl https://en.wikipedia.org/wiki/List_of_windmills_in_the_Netherlands | trim
```

Remember, `trim` is used only to make the output fit nicely in the book. As you can see, `curl` downloads the raw HTML returned by Wikipedia's server; no interpretation is being done and all the contents is immediately printed on standard output. Because of the URL, you'd think that this article would list all the windmills in the Netherlands. However, there are apparently so many windmills left, that each province has their own page. Fascinating.

By default, `curl` outputs a progress meter that shows how the download rate and the expected time of completion. Because this is written to standard error, and not standard output, it doesn't interfere when you add another tool to the pipeline. While this information can be useful when downloading very large files, I usually find it distracting, so I specify the `-s` option to *silence* this output.
...
And what do you know, there are apparently 234 windmills in the province of Friesland alone! I'll discuss `pup`, a handy tool for scraping websites, in more detail in Chapter 5.
...
In total, `curl` supports more than twenty protocols (see https://ec.haxx.se/protocols/protocols-curl). To download from an FTP server, which stands for File Transfer Protocol, you use `curl` the same way:
...
Or how about the DICT protocol:

```{console curl_dict}
curl -s dict://dict.org/d:windmill | trim
```

When downloading data from the Internet, however, the protocol most likely be HTTP, so the URL will start with either *http://* or *https://*.
...
If the original data set is very large or it's a collection of many files, the file may be (compressed) archive. Data sets which contain many repeated values (such as the words in a text file or the keys in a JSON file) are especially well suited for compression.

Common file extensions of compressed archives are: *.tar.gz*, *.zip*, and *.rar*. To decompress these, you would use the command-line tools `tar` [@tar], `unzip` [@unzip], and `unrar` [@unrar], respectively. There exists a few more, though less common, file extensions for which you would need yet other tools.
...
```{console tar_x}
tar -xzf logs.tar.gz #! enter=FALSE
```
...
Is seems that this archive contains a lot of files, and they are not inside a directory. In this case it's a good idea to first create a new directory using `mkdir` and extract those files there using the `-C` option:
...
Excellent. Now, I understand that you'd like to scrub and explore these log files, but that's for later.
...
`unpack` is part of the Data Science Toolbox. Below is the source of `unpack`. Although Bash scripting is not the focus of this book, it’s still useful to take a moment to figure out how it works.
...
As you can see, CSV by default is not too readable. You can pipe the data to a tool called `csvlook` [@csvlook], which will nicely format the data into a table. If the CSV data has no header then you need to add the `-H` option, otherwise the first line will be interpreted as the header.
...
By the way, the tools `in2csv`, `csvgrep`, and `csvlook` are part of CSVkit, which is collection of command-line tools to work with CSV data. CSVkit will be used quite often in this book because it has so many valuable tools. If you’re running the Data Science Toolbox, you already have CSVkit installed. Otherwise, see the appendix for instructions on how to install it.
...
Many companies have their data stored in a relational database. Just as with spreadsheets, it would be great if we could obtain that data from the command line.
...
Fortunately, there is a command-line tool called `sql2csv`, which is part of the CSVkit suite. Because it leverages the Python SQLAlchemy package, we can use one tool to execute queries on many different databases through a common interface, including MySQL, Oracle, PostgreSQL, SQLite, Microsoft SQL Server, and Sybase. The output of `sql2csv` is, as its name suggests, in CSV format.
...
`sql2csv` needs two arguments: `--db`, which specifies the database URL, of which the typical form is: `dialect+driver://username:password@host:port/database` and `--query`, which contains the `SELECT` query. For example, given an SQLite database that contains the standard data sets from R (see https://github.com/r-dbi/RSQLite/blob/master/inst/db/datasets.sqlite), I can select all the rows from the table `mtcars` and sort them by the `mpg` column as follows:
...
Some web APIs require you to authenticate before you can consume their output. There are several ways in which authentication can happen. Some web APIs use API keys while others use the OAuth protocol. News API is a great example. It allows you to search and retrieve news articles. Let's see what happens when you try to access this API without an API key:

```{console curl_nokey}
curl -s "http://newsapi.org/v2/everything?q=linux" | jq .
```

Well, that was to be expected. The part after the question mark, by the way, is where we pass any query parameters. That's also the place where you need to specify an API key. Below I insert my own API key by reading the file */data/secret/newsapi.org_apikey* using command substitution because I'd like to keep that a secret. You can get your own API key at https://newsapi.org.
...
Some web APIs return data in a streaming manner. This means that once you connect to it, the data will continue to pour in forever, until the connection is closed. A well-known example is the Twitter "firehose", which constantly streams all the tweets being sent around the world. Luckily, most command-line tools that we use also operate in a streaming matter, so that we also use this kind of data.
...
The command-line tool `sample` is used to close the connection after 10 seconds. The connection can also be closed manually by pressing ***Ctrl-C*** to send an interrupt. The output is saved to a file *wikimedia-stream-sample*. Let's take a peek using `trim`:
