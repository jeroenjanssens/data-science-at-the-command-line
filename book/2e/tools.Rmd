...
```{console alias}
type alias
man zshbuiltins | grep -A 10 alias | trim
...
```{console cd}
type cd
man zshbuiltins | grep -A 10 cd | trim
...
I use `chmod` in Chapter 4 to make a tool executable.
More information: https://www.gnu.org/software/coreutils.
...
```{console export}
type export
man zshbuiltins | grep -A 10 export | trim
...
Execute commands for each member in a list. In Chapter 8, I discuss the advantages of using `parallel` instead of `for`..
`for`

```{console for}
type for
man zshmisc | grep -EA 10 '^ *for ' | trim
...
rush run '6*7'
rush run -t 'starwars %>% filter(species == "Human") %>% select(name)'
```
...
(version 0.2.4)
by Jeroen Janssens (2021).
More information: https://github.com/jeroenjanssens/sample-stream.
...
shuf -ri 1-9 | paste -sd\  | fold | head | trim 5 40
```
